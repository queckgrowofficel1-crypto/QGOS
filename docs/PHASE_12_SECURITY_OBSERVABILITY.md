...
- Request correlation IDs across HTTP requests.
- Production-safe error handling and diagnostics.
- Security-focused HTTP headers and production configuration checks.
- Operational health and readiness verification.
...
- HTTP request completion is emitted as structured JSON through the NestJS logger with request ID, method, path, status code, and duration.
- Request logging deliberately excludes request bodies, authorization headers, and other credential material.
...
External infrastructure, credentials, monitoring vendors, backups, TLS, and legal/compliance controls remain deployment-owner prerequisites.
